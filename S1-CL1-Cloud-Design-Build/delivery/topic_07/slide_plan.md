...
> **Subtitle:** Build the network tier — the VPC your workload lives in, the rules around it, and how it's found by name

## Slides

### Opener
- [BESPOKE] Continuing the build
  - Topic 6 set the foundation: the account, identity, and the shared-responsibility line.
  - Now you build the network the workload sits inside.
  - Three parts: the virtual network (VPC + subnets), traffic control (security groups), and name resolution (DNS).
  - Still working to the supplied design — and still capturing evidence as you go.
  - For every AWS build task: watch the demo, then do it yourself.
  kicker: foundation done — now the network
  image: none
  notes:
...
- Kicker: the fabric your workload lives in
- [BESPOKE] Networking, the essentials
  - A network connects machines so they can share resources; it can be split into smaller pieces called subnets.
  - Every machine has an IP address — a unique numeric label (e.g. 10.0.0.17).
  - A CIDR block describes a range of addresses: 10.0.0.0/16 — the /number says how many bits are fixed.
  - Traffic moves between networks through routers / gateways.
  kicker: AWS · ACF M05 S5–S9
...
- [BESPOKE] The VPC — your private network in AWS
  - A VPC is a logically isolated section of AWS that you define — your own virtual network.
  - It belongs to one Region and can span Availability Zones; you size it with a CIDR block (largest /16, smallest /28; you can't change it later).
  - You divide the VPC into subnets — each in one AZ, classified public or private.
  kicker: AWS · ACF M05 S11 · ACA M07 S10
  image: none
  notes:
...
- [BESPOKE] Routing: gateways & route tables
  - A route table holds rules that direct a subnet's traffic; every table has a built-in local route for inside-the-VPC traffic.
  - Public subnet → an Internet Gateway (a 0.0.0.0/0 route to the IGW) gives direct internet access.
  - Private subnet → no direct internet path.
  - A NAT gateway lets private resources reach OUT (e.g. patching) without being reachable from the internet.
  kicker: AWS · ACA M07 S12–S16
...
- [EX] Public or private?
  - For each resource, decide which subnet it belongs in — public or private:
    - the database · a batch-processing job · the web/app server · a NAT gateway
  - Rule of thumb: anything that shouldn't be reachable from the internet goes private; only internet-facing entry points and the NAT gateway go public.
  - (AWS recommends even web/app tiers sit private, behind a load balancer.)
  timer: ~10 min
  image: none
  notes:
...
- [BESPOKE] The network you'll build
  - Open the Solution Design for this engagement (on the intranet) and study its network topology (§4.4) — you build to that.
  - VPC 10.0.0.0/16 (room to expand); DNS resolution + flow logs on.
  - Subnets in the first Availability Zone: public-egress-a (NAT only) · private-app-a (app + internal ALB) · private-data-a (database).
  - Internet Gateway is egress-only — patching via NAT; no inbound path from the internet.
  - Staff reach the service over the campus Site-to-Site VPN; route tables set per tier.
  kicker: work from the Solution Design
  image: none
  notes:
...
### C2 — Controlling traffic
- Teaches: [ICTCLD401 KE 9] · [ICTCLD401 PE 1]
- Kicker: who may talk to what
- [BESPOKE] What a firewall does
  - A firewall filters network traffic by rule — it decides what's allowed through.
  - Think of a security group like an apartment door: it only lets in the tenant with the right key — it protects one resource.
  - Traffic is identified by address + port: 443 = HTTPS, 1433 = SQL Server, 3389 = RDP.
  - Least privilege — open only the ports the job needs, only to who needs them.
  kicker: AWS · ACA M07 S23
  image: none
  notes:
...
- [BESPOKE] Security groups
  - A security group is a virtual firewall attached to a resource (an instance or interface).
  - Stateful — if you allow traffic in, the return traffic is automatically allowed out.
  - Allow-rules only (no deny). A new SG blocks all inbound until you add rules.
  - You can name another security group as the source — that's how you chain tiers (e.g. allow 443 only from the load balancer's SG).
  kicker: AWS · ACF M05 S33–S35 · ACA M07 S24
  image: none
  notes:
...
- [TABLE] The design's security groups
  kicker: least-privilege, chained tier-to-tier
...
- [DEMO] Build a VPC and its security groups
  - Create a public and a private subnet, each with its own route table.
  - Create an Internet Gateway, attach it to the VPC, and configure the internet route.
  - Create a NAT gateway, assign an Elastic IP, and add the private-subnet route.
  - Create a web-server security group and a database security group.
  source: ACA M07 · Creating a VPC (S30)
  image: none
  notes:
...
- [EX] Build the network fabric
  - In the AWS Academy lab, per the supplied Accounting design, build the network tier:
    - VPC 10.0.0.0/16; the three subnets (public-egress / private-app / private-data)
    - Internet Gateway + NAT gateway + per-tier route tables (egress-only)
    - the sg-alb → sg-app → sg-db security-group chain
  - Capture named evidence as you go — VPC, subnets, route tables, SG rules — Region visible.
  timer: ~25 min
  image: none
  notes:
...
### C3 — Found by name & proven to work
- Teaches: [ICTCLD401 PC 2.6] · [ICTCLD401 KE 10] · [ICTCLD401 PE 3]
- Kicker: DNS · connectivity testing
- [BESPOKE] How DNS works — and Route 53
  - DNS translates a name (ledgerline.yat.internal) into the IP address computers actually use.
  - A resolver asks the DNS hierarchy and gets the address back — we use names, not IPs, because addresses change.
  - Amazon Route 53 is AWS's highly available DNS service.
  - Public hosted zone = internet-facing names; private hosted zone = names that resolve only inside your VPC.
  kicker: AWS · ACF M05 S49–S50
  image: reuse 07-route53-dns-flow.png
  notes:
...
- [BESPOKE] The design's name & certificate
  - Ledgerline gets an internal hostname in a private hosted zone — staff-only, over the VPN; no public internet name.
  - An ACM TLS certificate secures HTTPS to that name.
  - The exact hostname + certificate domain is implementer decision C8 — confirm it with YAT ICT.
  kicker: the supplied design · decision C8
  image: none
  notes:
...
- [BESPOKE] Prove it works — test connectivity
  - After building, confirm reachability: external access in (reach the service over the VPN) and access between tiers (app → db).
  - Reachability Analyzer traces the path and names the blocking component when something can't connect.
  - VPC flow logs show ACCEPT / REJECT — a REJECT is usually a security-group rule or a missing route.
  - Fix it and retest; capture the test as evidence. (Full app → db testing completes in Topic 8, once compute exists.)
  kicker: AWS · ACA M07 S45, S41
  image: none
  notes:
...
- [DEMO] Create a DNS record & test reachability
  - In Route 53, create / confirm the private hosted zone for the internal domain.
  - Add the record for the Ledgerline name (and note the ACM certificate).
  - From a test point, resolve the name and check reachability.
  - If it's blocked, read the flow logs / Reachability Analyzer and fix the SG or route.
  image: none
  notes:
...
- [EX] Configure DNS + test connectivity
  - In the lab, finish the network tier:
    - create the private hosted zone + record for the internal hostname (note the ACM cert)
    - run reachability tests for what exists so far; record the result and any fix you made
  - Capture the DNS config and the test result as evidence.
  timer: ~15 min
  image: none
  notes:
