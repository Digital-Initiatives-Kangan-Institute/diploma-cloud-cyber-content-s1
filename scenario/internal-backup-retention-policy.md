# YAT College — Backup and Retention Policy
...
**Source status:** ✅ Authored 2026-05-23 (Claude). Policy content drafted using Standards for RTOs 2015 records-retention defaults and marked **TBD** where specifics warrant Tim's confirmation.
...
It works in conjunction with the operational backup and recovery process documented at `internal-backup-recovery-process-S1-CL1-AT1.md` (which describes *how* backups currently run) and the Privacy / Data Handling Policy (which sets the framework within which personal information is held).
...
## 4. Retention periods **TBD**

Retention periods below are Claude proposals based on Standards for RTOs 2015 and common Australian RTO practice. **TBD** confirm against YAT's actual obligations.
...
- A restoration test is performed at least **quarterly** **TBD** to verify backups are recoverable. A subset of the catalogue is restored to a non-production environment and integrity-checked.
- Results of restoration tests are recorded by the ICT Service Desk and reported to the ICT Manager.
- Material failures (unable to restore, restored data corrupt) trigger an incident response per `internal-security-and-incident-response.md`.
...
- **System Management server:** provides the technical infrastructure for the on-prem backup process (per the Current ICT Environment).
