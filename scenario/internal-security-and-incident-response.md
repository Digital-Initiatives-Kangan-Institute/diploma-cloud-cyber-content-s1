# YAT College — Security Baseline and Incident Response
...
**Source status:** ✅ Authored 2026-05-23 (Claude). Policy content drafted using ACSC Essential Eight + Notifiable Data Breaches scheme defaults and marked **TBD** where specifics warrant Tim's confirmation.
...
- **Network segmentation** — student zone vs staff zone (per the Current ICT Environment description)
...
- **Physical security** — server room is physically secured and air-conditioned (per the Current ICT Environment description)
...
2. **Triage.** ICT Service Desk performs initial triage and assigns severity. Severity 1 and 2 incidents are escalated to the ICT Manager immediately. The student / MTS consultant on engagement participates in triage for cloud-environment incidents in scope of their engagement.
...
5. **Recover.** Restore service — restore from backups if needed (per the Backup and Recovery Process), re-enable affected systems, verify integrity.
...
- **Communications lead** **TBD** (e.g. Compliance Manager): coordinates internal and external communications during a Severity 1 or 2 incident.
