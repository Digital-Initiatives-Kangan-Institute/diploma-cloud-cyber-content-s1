...
Reuses the branding helpers from build_bc_template.py so it carries identical YAT brand
...
Usage:  python scripts/build_bc_exemplar.py [output.docx]
...
def uoc(doc, text):
    ...
    r.italic = True
    r.font.size = Pt(8.5)
    r.font.color.rgb = RGBColor.from_string(bc.TEAL)
...
def etable(doc, headers, rows, widths=None):
    """Filled table — header teal/white, body normal text, money right-aligned."""
    t = doc.add_table(rows=1, cols=len(headers))
    t.alignment = WD_TABLE_ALIGNMENT.CENTER
    hdr = t.rows[0].cells
    for i, h in enumerate(headers):
        bc.shade_cell(hdr[i], bc.TEAL)
        bc.set_cell_borders(hdr[i])
        run = hdr[i].paragraphs[0].add_run(h)
        run.bold = True
        run.font.color.rgb = RGBColor.from_string(bc.WHITE)
        run.font.size = Pt(9)
    for row in rows:
        cells = t.add_row().cells
        for i, val in enumerate(row):
            bc.set_cell_borders(cells[i])
...
def build(path):
    ...
    uoc(doc, "contributes to [ICTICT517 PC 1.4]")
    para(doc, "YAT's Learning Management System (DOODLE) runs on a single, end-of-supportable-life "
    ...
    para(doc, "The five-year direct cost of the two options is comparable (~$447,300 on-premises vs "
    ...
    para(doc, "The principal risks of the cloud option — staff upskilling, cost control, and a safe "
    ...
    uoc(doc, "[ICTICT517 AC 4]")
    para(doc, "This Business Case is prepared by MP Tech Solutions (MTS) for the YAT Board, via the YAT "
    ...
    uoc(doc, "[ICTICT517 PC 1.1] · [ICTICT517 PE 1]")
    para(doc, "YAT's ICT Strategic Plan sets goals to ensure reliable ICT services, respond quickly to "
    ...
    para(doc, "Against the industry environment, the direction is well aligned: Australian RTOs and the "
    ...
    para(doc, "Where YAT's plan is more ambitious than its current posture is the 99.9% availability "
    ...
    uoc(doc, "[ICTICT517 PC 1.2] · [ICTICT517 PE 2]")
    para(doc, "YAT's network is resilient at the infrastructure layer (no single point of failure, "
    ...
    h1("5. Gap Analysis")
    uoc(doc, "[ICTICT517 PC 1.3] · [ICTICT517 PE 3]")
    etable(doc,
    ...
    para(doc, "The gaps cluster around resilience, recovery, capacity, and ownership. An on-premises "
    ...
    uoc(doc, "[ICTICT517 PC 2.1] · [ICTICT517 PC 2.2] · [ICTICT517 KE 3] · [ICTCLD401 PC 1.8] · [ICTCLD502 PC 1.2]")
    h2("6.1 Workload definition")
    para(doc, "The LMS serves ~860 users (≈800 students + 60 staff), with 200–300 typical concurrent "
    ...
    para(doc, "Each option is evaluated with a five-year cost-benefit analysis (§7), a quantified "
    ...
    etable(doc, ["", "Option A — In-house", "Option B — Cloud (AWS)"],
    ...
    uoc(doc, "[ICTICT517 PC 2.1] · [ICTICT517 KE 3] · [ICTICT517 FS Numeracy] · [ICTCLD401 KE 4] · [ICTCLD502 KE 3] · [ICTCLD401 KE 1]")
    para(doc, "A five-year CBA comparing the two options (AUD ex GST). Detailed line items are in "
    ...
    h2("7.1 Assumptions")
    etable(doc, ["Assumption", "Value", "Used as-is?"],
    ...
    etable(doc, ["", "Option A", "Option B", "Delta (B − A)"],
    ...
    para(doc, "On direct cost the options are within ~1.3% over five years. Option B carries a higher "
    ...
    para(doc, "Sensitivity 1 — teaching-hours downtime proportion 30% → 50%: the avoided-downtime "
              "benefit rises from ~$69,000 to ~$115,000 over five years, widening Option B's net "
              "advantage. The recommendation is reinforced.")
    para(doc, "Sensitivity 2 — MTS migration labour $60,000 → $80,000: Option B's five-year direct cost "
    ...
    uoc(doc, "[ICTICT517 PC 2.1] · [ICTICT517 PC 2.2] · [ICTICT517 PE 5]")
    h2("8.1 Intangibles comparison")
    etable(doc, ["Factor", "Option A — In-house", "Option B — Cloud (AWS)"],
    ...
    etable(doc, ["Risk", "Likelihood", "Impact", "Mitigation"],
    ...
    h1("9. Recommendation")
    uoc(doc, "[ICTICT517 PC 2.3] · [ICTICT517 PE 4] · contributes to [ICTICT517 PC 3.1]")
    para(doc, "Recommended option: Option B — Cloud migration to AWS. Over five years the two options "
    ...
    h1("10. Action Plan")
    uoc(doc, "[ICTICT517 PC 3.1] · [ICTICT517 PC 3.2] · [ICTICT517 PE 6] · [ICTICT517 KE 1]")
    h2("10.1 Prioritised changes")
    etable(doc, ["#", "Change", "Priority rationale"],
    ...
    etable(doc, ["Phase", "Activities", "Start", "Duration", "Dependencies", "Owner"],
    ...
    para(doc, "Scheduling avoids month-end and the two-week assessment windows; the cutover runs in an "
              "approved maintenance window with a tested rollback.")
    h2("10.3 Standards, targets, and success metrics")
    etable(doc, ["Aspect", "Standard / target", "How measured"],
    ...
    para(doc, "Each phase applies an AWS Well-Architected review at its design milestone, infrastructure-"
    ...
    para(doc, "Phases 2–4 raise change requests; the Phase 4 cutover is a high-risk change requiring "
    ...
    uoc(doc, "[ICTICT517 PC 3.3]")
    para(doc, "We ask the board today to: (1) approve Option B — cloud migration to AWS — as the "
    ...
    h1("Sign-off")
    uoc(doc, "[ICTICT517 PC 3.3]")
    etable(doc, ["Role", "Name", "Date", "Signature"],
    ...
    uoc(doc, "[ICTICT517 FS Numeracy] · [ICTICT517 KE 3] · [ICTCLD401 KE 4] · [ICTCLD502 KE 3]")
    h2("A1.1 Option A — In-house renewal")
    para(doc, "One-off capital (Year 1):")
    etable(doc, ["Item", "Cost"],
           ...
           widths=[11.5, 4.0])
    para(doc, "Recurring operational (per year):")
    etable(doc, ["Category", "Item", "Annual cost"],
    ...
    para(doc, "One-off project costs (Year 1):")
    etable(doc, ["Item", "Cost"],
           ...
           widths=[11.5, 4.0])
    para(doc, "AWS direct recurring (per year, indicative — ap-southeast-2):")
    etable(doc, ["Category", "Service / item", "Annual cost"],
           ...
           widths=[3.5, 8.0, 4.0])
    para(doc, "Staff + ongoing external support (per year, Years 2–5):")
    etable(doc, ["Item", "Annual cost"],
    ...
    para(doc, "Underlying knowledge as applied to the work presented above.")
    h2("A2.1 Selection-style questions")
    etable(doc, ["Question", "Response"],
