...
Same shape as the AT3 assessment workbook, on the public website rather than the assessed system. The
...
WHAT ELSE DIFFERS, and why an answer here does not transfer:
  ...
  the before state  ONE EC2 instance with no load balancer and no Auto Scaling group. The
                    assessed system's baseline already has both. So the first improvement here
                    is building a tier, not widening one.
  ...
  matters           event is ANONYMOUS PUBLIC traffic when courses are advertised — not
                    the assessed system's staff transaction load inside an enrolment window. Same weeks
...
TASKS = [
    dict(n=1, title="Deploy the baseline environment",
         ...
         clicks=["Start the AWS Academy Learner Lab and open the console. Check the region reads "
                 ...
                 "placeholder page should load — this one is public, unlike the assessed system.",
